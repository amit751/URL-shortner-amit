...
function readFileSUCSESS(counterData , bin , url){
...
function readFileFail(err){
    return res.send(err);
    
}

function onFullfild(data ,url){
    const bin = JSON.parse(data);
    if(url in bin){
    ...
    return fetch(url).then((response)=>{
        if(response.ok){
            ...
            .then((counterData)=>{
                readFileSUCSESS(counterData , bin , url);
            } , (err)=>{
                readFileFail(err);
            });
            return;
